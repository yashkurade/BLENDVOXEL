...
class VoxelGridProps(bpy.types.PropertyGroup):
    ...
    current_layer: IntProperty(name="Active Layer", default=0, min=0)
...
class VOXEL_OT_erase_voxel(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        if event.type == 'MOUSEMOVE' and getattr(self, 'dragging', False):
            region = context.region
            rv3d = context.region_data
            coord = event.mouse_region_x, event.mouse_region_y
            ...
            plane_normal = {
                ...
                'XZ': Vector((0, 1, 0)),
                'YZ': Vector((1, 0, 0)),
            }.get(props.orientation, Vector((0, 0, 1)))
            plane_point = {
                'XY': Vector((0, 0, layer)),
                'XZ': Vector((0, layer, 0)),
                'YZ': Vector((layer, 0, 0)),
            }.get(props.orientation, Vector((0, 0, layer)))
