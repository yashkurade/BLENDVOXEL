...
class VoxelGridPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        op = layout.operator("voxel.join_and_merge", text="Optimise Voxels")
        op.shell_thickness = 0.01
...
class VOXEL_OT_join_and_merge(bpy.types.Operator):
    """Join voxels and keep only outer shell (remove inner mesh)"""
    bl_idname = "voxel.join_and_merge"
    bl_label = "Join & Skin Only"
    bl_options = {'REGISTER', 'UNDO'}

    shell_thickness: bpy.props.FloatProperty(
        name="Shell Thickness",
        default=0.01,
        min=0.0001,
        description="Thickness for outer shell (Solidify modifier)"
    )

    def execute(self, context):
        selected = context.selected_objects
        if len(selected) < 1:
            self.report({'WARNING'}, "Select at least one voxel object")
            return {'CANCELLED'}
        ...
        # Shrink using solidify (negative thickness)
        solid = duplicate.modifiers.new(name="Shrink", type='SOLIDIFY')
        solid.thickness = -abs(self.shell_thickness)
        solid.offset = 1
        bpy.context.view_layer.objects.active = duplicate
        bpy.ops.object.modifier_apply(modifier=solid.name)

        # Boolean Difference
        bool_mod = original.modifiers.new(name="RemoveInner", type='BOOLEAN')
        ...
        self.report({'INFO'}, "Internal geometry removed.")
        return {'FINISHED'}
